...
const UnitMaster = (props: Props) => {
  ...
  const getList = () => {
    axios.get(`https://localhost:59928/api/Master/GetUnit/-1`)
      .then(res => {
        ...
        // const element = res.data.result[index]['vehileID'];
         //console.log(element);
         arr.push({ sr_no: index+1, id: res.data.result[index]['unitId'], UnitName:  res.data.result[index]['unitName'], UnitShortname:  res.data.result[index]['unitShortname']});
...
const routeChangeEdit =  (edit_id: any) =>  { 
  ...
  let path = `/master/UnitMasterEdit`; 
  navigate(path,{
  ...
  return (
    <div>
            ...
            <h3 style={{textAlign:'center', marginTop:0 }}>Unit Master</h3> 
      <h3 style={{marginLeft: 90}}>Unit Master</h3>
