...
const DepartmentMaster = (props: Props) => {
  ...
  const getList = () => {
    axios.get(`https://localhost:59928/api/Master/GetDepartment/-1`)
      .then(res => {
        ...
        // const element = res.data.result[index]['vehileID'];
         //console.log(element);
         arr.push({ sr_no: index+1, id: res.data.result[index]['departmentId'], DepartmentName:  res.data.result[index]['departmentName'], DepartmentShortname:  res.data.result[index]['departmentShortname']});
...
const routeChangeAdd = () =>{ 
  let path = `/employeeinfo/DepartmentMasterAdd`; 
  navigate(path);
}
...
const routeChangeEdit =  (edit_id: any) =>  { 
  ...
  let path = `/employeeinfo/DepartmentMasterEdit`; 
  navigate(path,{
  ...
  return (
    <div>
            ...
            <h3 style={{textAlign:'center', marginTop:0 }}>Department Master</h3> 
      <h3 style={{marginLeft: 90}}>Department Master</h3>
