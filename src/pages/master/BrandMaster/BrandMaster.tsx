...
const BrandMaster = (props: Props) => {
  ...
  const getList = () => {
    axios.get(`https://localhost:59928/api/Master/GetBrand/-1`)
      .then(res => {
        ...
        // const element = res.data.result[index]['vehileID'];
         //console.log(element);
         arr.push({ sr_no: index+1, id: res.data.result[index]['brandId'], brandCode:  res.data.result[index]['brandCode'], brandname:  res.data.result[index]['brandname'], brandshortname:res.data.result[index]['brandshortname']});
...
const routeChangeAdd = () =>{ 
  let path = `/master/BrandMasterAdd`; 
  navigate(path);
}
...
const routeChangeEdit =  (edit_id: any) =>  { 
  ...
  let path = `/master/BrandMasterEdit`; 
  navigate(path,{
  ...
  return (
    ...
    <div>
      <Box height={60} />
            ...
            <h3 style={{textAlign:'center', marginTop:0 }}>BRAND MASTER</h3> 
      <h3 style={{marginLeft: 90}}>BRAND Master</h3>
