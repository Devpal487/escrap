...
const TaxMaster = (props: Props) => {
  ...
  const getList = () => {
    axios.get(`https://localhost:59928/api/Master/GetTax/-1`)
      .then(res => {
        ...
        // const element = res.data.result[index]['vehileID'];
         //console.log(element);
         arr.push({ sr_no: index+1, id: res.data.result[index]['taxId'], TaxName:  res.data.result[index]['taxName'], TaxPercentage:  res.data.result[index]['taxPercentage'], EffectiveDate:  res.data.result[index]['effectiveDate']});
...
const routeChangeEdit =  (edit_id: any) =>  { 
  ...
  return (
    <div>
            ...
            <h3 style={{textAlign:'center', marginTop:0 }}>Tax Master</h3> 
      <h3 style={{marginLeft: 90}}>Tax Master</h3>
