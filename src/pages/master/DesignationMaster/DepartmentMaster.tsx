...
const DesignationMaster = (props: Props) => {
  ...
  const getList = () => {
    axios.get(`https://localhost:59928/api/Master/GetDesignation/-1`)
      .then(res => {
        ...
        // const element = res.data.result[index]['vehileID'];
         //console.log(element);
         arr.push({ sr_no: index+1, id: res.data.result[index]['designationId'], DesignationName:  res.data.result[index]['designationName'], DesignationCode:  res.data.result[index]['designationCode']});
...
const routeChangeAdd = () =>{ 
  let path = `/employeeinfo/DesignationMasterAdd`; 
  navigate(path);
}
...
const routeChangeEdit =  (edit_id: any) =>  { 
  ...
  let path = `/employeeinfo/DesignationMasterEdit`; 
  navigate(path,{
  ...
  return (
    <div>
            ...
            <h3 style={{textAlign:'center', marginTop:0 }}>Designation Master</h3> 
      <h3 style={{marginLeft: 90}}>Designation Master</h3>
